...
StatesPathFinder::StatesPathFinder(const core::ProblemConstPtr_t& problem,
  ...
  core::PathProjectorPtr_t pathProjector(
      core::pathProjector::Progressive::create(inStateProblem_, 1e-2));
...
int StatesPathFinder::solveStep(std::size_t wp) {
  ...
  if (status == Solver_t::SUCCESS) {
    ...
    core::ConfigValidationsPtr_t configValidations =
        problem_->configValidations();
    core::ConfigValidationsPtr_t configValidations2 =
        core::ConfigValidations::create();
    core::CollisionValidationPtr_t colValidation =
        core::CollisionValidation::create(problem()->robot());
...
bool StatesPathFinder::solveOptimizationProblem() {
  ...
  while (wp <= d.solvers.size()) {
    ...
    while (nTriesDone[wp] >= nTriesMax) {
      if (wp == 1) {
        if (nTriesDone[wp] < nTriesMax1) break;
    ...
    if (wp > 1 && nFails >= nFailsMax) {
      for (std::size_t k = 2; k <= d.solvers.size(); k++) nTriesDone[k] = 0;
      wp = 1;
...
void StatesPathFinder::oneStep() {
  ...
  try {
    ...
    q1 = ConfigurationPtr_t(new Configuration_t(configSolved(idxConfigList_)));
    q2 = ConfigurationPtr_t(
        new Configuration_t(configSolved(idxConfigList_ + 1)));
    ...
    std::ostringstream oss;
    oss << "Error " << e.what() << "\n";
    oss << "Solution " << idxSol_ << ": Failed to build path at edge "
        << idxConfigList_ << ": ";
